...
public class Utils {
	...
	public static String formatTitleDesc(String title){
		...
		if (tmp.length() < 26){
			...
			if (t.length() < 26){
				...
				newTitle = newTitle + tmp.substring(0,26);
			}
		}
	...
	private static void readNotesAndRenameFiles(File[] files) {
		for (File file : files) {
			if (file.isDirectory()) {
				System.out.println("Directory: " + file.getName());
				readNotesAndRenameFiles(file.listFiles()); // Calls same method again.
			} else {
				String abPath = file.getAbsolutePath().replace("\\", "/");
...
//				System.out.println("File: " + abPath);
			...
			while((thisLine = br.readLine()) != null){
				...
				if (Constants.MAPKEY_MAIN.equals(currentKey)){
					...
					if (lineAry.length == 2 && new File(lineAry[1]).exists()){
						imgPath = lineAry[1];
						newImgPath = FilenameUtils.getFullPath(imgPath) + codePrefix + "_M_" + FilenameUtils.getName(imgPath);
					...
					if (lineAry.length == 5 && new File(lineAry[4]).exists()){
						imgPath = lineAry[4];
						newImgPath = FilenameUtils.getFullPath(imgPath) + codePrefix + "_C_" + FilenameUtils.getName(imgPath);
						newLine = thisLine.replace(imgPath, newImgPath);
					}
					...
					if (lineAry.length == 2 && new File(lineAry[1]).exists()){
						imgPath = lineAry[1];
						newImgPath = FilenameUtils.getFullPath(imgPath) + codePrefix + "_D_" + FilenameUtils.getName(imgPath);
						newLine = thisLine.replace(imgPath, newImgPath);
					}
				}
	...
	private static void updateNotesFile(String imgOriPath, List<String> croppedImgList){
		...
		String notesNewPath = imgOriPath.substring(0, imgOriPath.indexOf("details")).replace("\\", "/") + "note_new.txt";
	...
	public static void resizeInWidth(String filePath, int targetWidth) {
		...
		try {
			...
			if (image.getWidth() == targetWidth) {
				...
				BufferedImage newImg = Scalr.resize(image, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.FIT_TO_WIDTH, targetWidth, 0,
						Scalr.OP_BRIGHTER);
...
//				bw.newLine();
		...
		for (String[] oAry : linkList){
			if (map.containsKey(oAry[2])){
				...
				System.out.println(oAry[0].replaceAll("正品", "") + Constants.TM_SEPERATOR + menusStr.toString().replaceAll("正品", "") + Constants.TM_SEPERATOR + oAry[2]);
				map.remove(oAry[2]);
			}
		}
	}
...
//		Iterator<Entry<String, Set<String>>>  items = map.entrySet().iterator();
//		while(items.hasNext()){
...
////				System.out.println("----------------------------------------");
////				System.out.println(ary[0] + Constants.TM_SEPERATOR + ary[2] + Constants.TM_SEPERATOR + ary[3] + Constants.TM_SEPERATOR + ary[4]);
//				String line = ary[0] + Constants.TM_SEPERATOR + ary[2] + Constants.TM_SEPERATOR + ary[3] + Constants.TM_SEPERATOR + ary[4];
//				int k = 0;
...
//		}
		...
		for (String[] ary : rows){
			System.out.println(ary[0] + Constants.TM_SEPERATOR + ary[1] + Constants.TM_SEPERATOR + ary[2] + Constants.TM_SEPERATOR + ary[3] + Constants.TM_SEPERATOR + ary[4]);
		}
...
//					rows.add(new String[]{fldIDMap.get(proId), menuPath, proId, newTitle, newProId});	
//				}
...
//			if (map.containsKey(proId) && map.get(proId).size() > 1){
//				System.out.println("----------------------------------------");
//				System.out.println(ary[0] + Constants.TM_SEPERATOR + ary[2] + Constants.TM_SEPERATOR + ary[3]);
//				
//				int k = 0;
//				for (String s : map.get(proId)){
//					if (k++ > 0){
//						String[] sAry = s.split(Constants.TM_SEPERATOR);//length:4
//						System.out.println(ary[2] + Constants.TM_SEPERATOR + sAry[3].substring(sAry[3].indexOf("?id=") + 4));
//					}
//				}
//				
//				map.remove(proId);
...
//		});
		for (String[] oAry : rows){
			if (menuMap.containsKey(oAry[2])){
				...
				System.out.println(oAry[2] + Constants.TM_SEPERATOR + oAry[3] + Constants.TM_SEPERATOR + newIdStr.toString());
	...
	private static void printProductsDifferenceList(String oldLinkPath, String newLinkPath){
		...
		for (String id : newMenusClone){
			...
			System.out.println(menuName + "/" +newCode + Constants.TM_SEPERATOR + formatMenuList(sortedList) + Constants.TM_SEPERATOR + id);
		}
		
		System.out.println("*********PRODUCTS REMOVED********");
		for (String id : oldMenusClone){
			System.out.println(formatMenuList(convertToSortedList(proMenuMapOld.get(id))) + Constants.TM_SEPERATOR + id);
		}
		...
		for (String id : oldMenus){
			...
			if (sortedNewMenusPerPro != null && !sortedOldMenusPerPro.toString().equals(sortedNewMenusPerPro.toString())){
				System.out.println(formatMenuList(sortedOldMenusPerPro) + " >>>> " + formatMenuList(sortedNewMenusPerPro) + Constants.TM_SEPERATOR + id);				
			}
		}
...
//				String proId = ary[1];
				...
				String newLine = foldPath + Constants.TM_SEPERATOR  +  menuPath + Constants.TM_SEPERATOR + proId + Constants.TM_SEPERATOR + title;
	...
	public static String formatMenuName(String menuName){
		return filterNonBreakSpace(menuName.replaceAll("正品", "").replaceAll("蓝球", "篮球").replaceAll("派对游戏 皮纳塔Pinata", "皮纳塔 Pinata"));
	}
...
//		System.out.println(Utils.formatTitleDesc(s));
		//System.out.println(Utils.generateRandomPrefix() + "" +  Utils.formatNumber(90));
...
//		Utils.matchFolderWithProID("D:/Taobao/Tools/links_all.txt", "D:/Taobao/Tools/upload_success_all.txt");
...
//		https://upload.taobao.com/auction/container/publish.htm?catId=50003854&itemId=559668807460
		//从URL中获取CATEGORY ID
//		String url = "https://upload.taobao.com/auction/container/publish.htm?catId=50003854&itemId=559668807460";
...
//		Utils.shrinkProductLinkList("d:/Taobao/tools/links_all.txt");
	}
}
